...
describe("authorizeOptionalUser", () => {
  ...
  it("rejects (does not silently downgrade to guest) a signed-in user who must still change their password", async () => {
    const user = await makeUser({ mustChangePassword: true, accountStatus: "PENDING_FIRST_LOGIN" });
    const result = await authorizeOptionalUser(user.clerkUserId);
  ...
  it("rejects a Clerk session with no matching Prisma row rather than treating it as guest", async () => {
    const result = await authorizeOptionalUser("clerk-id-with-no-app-user-row");
    expect(result).toMatchObject({ ok: false, reason: "account-not-found" });
  });
});
