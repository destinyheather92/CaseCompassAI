...
describe("RemoveSavedItemButton", () => {
  beforeEach(() => {
  ...
  it("sends a DELETE for the saved item and refreshes the page on success", async () => {
    const user = userEvent.setup();
    render(<RemoveSavedItemButton savedItemId="s1" />);

    await user.click(screen.getByRole("button", { name: /remove/i }));

    await waitFor(() => {
      expect(fetchMock).toHaveBeenCalledWith("/api/dashboard/saved-resources/s1", expect.objectContaining({ method: "DELETE" }));
