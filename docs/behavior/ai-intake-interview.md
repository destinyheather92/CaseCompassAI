...
- Guests may complete the entire interview without an account. A guest session's high-entropy cuid `sessionId` is its access credential — the same trust model as e.g. a Stripe Checkout session token, documented explicitly here rather than left implicit.
- A signed-in user who is disabled, locked, or must still change their temporary password is never silently treated as a guest — `lib/auth/authorization.ts`'s `authorizeOptionalUser()` rejects them outright (see `tests/integration/authorization/authorize-optional-user.test.ts`).
